...
import { ExamExportDataDTO, WordPayloadDTO } from '../dtos/exporter.dto';
...
import { BuildExamHeader } from 'src/exporter/infrastructure/utils/Build/buildExamHeader';
...
export class ExporterService {
  async generateWordFile(data: WordPayloadDTO): Promise<Buffer> {
    ...
    const imageQuestionCache: Record<string, Buffer> =
      await getQuestionImageBuffer(
        Object.values(data.questionsSorted).map((question) =>
          question.map((q) => ({
            id: q.id,
            question: q.question,
            options: q.options,
  ...
  async generateExamWordFile(data: ExamExportDataDTO[]) {
    const questionType = {
      'Nhiều lựa chọn': 'Câu trắc nghiệm nhiều phương án lựa chọn',
      'Đúng sai': 'Câu trắc nghiệm đúng sai',
      'Trả lời ngắn': 'Câu trắc nghiệm trả lời ngắn',
      'Tự luận': 'Câu tự luận',
    };
    const children: any[] = [];
    ...
    BuildExamHeader(children);

    const imageQuestionCache: Record<string, Buffer> =
      await getQuestionImageBuffer(data.map((d) => d.questions));

    for (const [index, d] of data.entries()) {
      // Phần X
      children.push(
        new Paragraph({
          text: `Phần ${index + 1}. ${d.questionType}`,
          heading: HeadingLevel.HEADING_2,
        }),
      );

      // Câu hỏi
      for (const [index, q] of d.questions.entries()) {
        const currentQuestion = BuildQuestion(
          index,
          q.question,
          regex,
